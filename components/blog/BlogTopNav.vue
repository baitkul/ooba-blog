<template>
  <div
        ...
        >
          ...
          >
            <div v-show="menuVisible" class="bg-white">
              ...
              <NuxtLink to="/" class="flex items-center px-4 text-base border-t border-gray-300 h-14">
                Главная
              </NuxtLink>
              <NuxtLink
                ...
                :key="idx"
                :to="'/blogs/'+ menu.slug"
                class="flex items-center px-4 text-base border-t border-gray-300 h-14"
              >
                {{ menu.title }}
              </NuxtLink>
            </div>
    ...
    <div
      class="fixed left-0 right-0 z-10 items-center justify-center delay-100 bg-gray-100 shadow-md opacity-0 container-auto"
      style="top: -100%; height: 80px; padding: 10px 20px"
      :style="inputActive ? 'top: 80px; opacity: 1' : '' ">
      <form class="flex justify-center w-full" @submit.prevent="submit">
        <input
          ref="inputForm"
          v-model.lazy="input"
          class="px-2 py-3 border-b-2 border-pink-600 outline-none w-80 bg-none focus:outline-none focus-within:outline-none"
